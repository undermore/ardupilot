...
float Copter::get_pilot_desired_climb_rate(float throttle_control)
{
    ...
    if (throttle_control < deadband_bottom) {
        ...
        // above the deadband
        desired_rate = g.my_pilot_speed_up * (throttle_control-deadband_top) / (1000.0f-deadband_top);
    }else{
        // must be in the deadband
        desired_rate = 0.0f;
    }
...
{
    if (g2.my_pilot_speed_dn == 0) {
        return abs(g.my_pilot_speed_up);
    } else {
        return abs(g2.my_pilot_speed_dn);
    }
}
